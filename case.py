...
INTERNAL_W = max(BASE_WIDTH, REAR_LAYOUT_W)
INTERNAL_L = PID_BODY_D + GAP_PID_SSR + SSR_L + GAP_SSR_REAR
...
# -- Positioning Logic --
porch_x = -INTERNAL_W/2 + PORCH_EXT_W/2 + 5.0

space_start_x = porch_x + PORCH_EXT_W/2 
space_end_x = INTERNAL_W/2 - BOSS_SIZE
cable_group_center_x = space_start_x + (space_end_x - space_start_x)/2

cable_1_x = cable_group_center_x - CABLE_SPACING/2
cable_2_x = cable_group_center_x + CABLE_SPACING/2

# -- Ghost Positions --
pid_center_y = -INTERNAL_L/2 + PID_BODY_D/2
ssr_center_y = -INTERNAL_L/2 + PID_BODY_D + GAP_PID_SSR + SSR_L/2
term_center_x = porch_x 
...
pid_body_ghost = Location((0, pid_center_y, pid_pos_z)) * pid_body_ghost

bezel_pos_y = -INTERNAL_L/2 - PID_BEZEL_D/2
pid_bezel_ghost = Box(PID_BEZEL_W, PID_BEZEL_D, PID_BEZEL_H)
pid_bezel_ghost = Location((0, bezel_pos_y, pid_pos_z)) * pid_bezel_ghost
...
ssr_ghost = Location((0, ssr_center_y, ssr_pos_z)) * ssr_ghost

term_ghost = Box(TERM_W, TERM_D, TERM_H)
term_ghost = Location((term_center_x, term_center_y, term_pos_z)) * term_ghost
...
with BuildPart() as body:
    ...
    with Locations((0, ssr_center_y, hole_top_z)): 
    ...
    # G. Front Cutout
    with Locations((0, -BOX_L/2, pid_pos_z)):
