...
PORCH_EXT_W = TERM_W + 20 
PORCH_EXT_D = TERM_D + 6 + PORCH_FREE_SPACE 
PORCH_WALL_THICKNESS = 6.0 
...
with BuildPart() as body:
    ...
    # Bottom Chamfer
    bottom_edges = body.edges().filter_by(lambda e: abs(e.center().Z) < 0.1)
    ...
    with BuildSketch(Plane.XY.offset(porch_floor_z)):
        with Locations((porch_x, porch_center_y)):
            ...
            with Locations((0, -2)): 
                Rectangle(PORCH_EXT_W - 2*PORCH_WALL_THICKNESS, PORCH_EXT_D + 4, mode=Mode.SUBTRACT) 
    extrude(amount=PORCH_WALL_H)
    
    porch_hole_spacing = PORCH_EXT_W - PORCH_WALL_THICKNESS 
    with Locations((porch_x, porch_center_y, porch_floor_z + PORCH_WALL_H)):
        with Locations((porch_hole_spacing/2, 0), (-porch_hole_spacing/2, 0)):
    ...
    # I. C14 INLET (PRIMITIVE CONSTRUCTION METHOD)
    ...
    # L. Front Vents (FAIL-SAFE SOLID BOOLEAN METHOD)
    # Position: Centered on SSR X, Front Wall Y (-BOX_L/2), Mid-height Z
    front_vent_z = BOX_H / 2 
    with Locations((ssr_center_x, -BOX_L/2, front_vent_z)):
        with GridLocations(6, 0, 5, 1): # 5 vents, 6mm pitch
            # Box Dimensions:
            # X = 3.0 (Slot Width)
            # Y = 10.0 (Cutting Depth - punches through wall)
            # Z = 25.0 (Slot Height)
...
assembly = Compound(children=[body.part, lid_moved, term_cover_moved, washer_moved, pid_body_ghost, ssr_ghost, mains_ghost])
